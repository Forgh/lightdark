...
public class ControlerProjectiles {
	...
	public void gererCollision(Projectile p, float delta){
		...
		while(i< monde.getAnime().size && ok){
			if(monde.getPerso().getForm() == Form.SHADOWFORM && monde.getAnime().get(i) instanceof Animal && persoRect.overlaps(monde.getAnime().get(i).getCadre())) {
				if(monde.getPerso().getEtat()!=Dark.TAMING) {
					System.out.println("[DEBUG] Shadow Taming" + monde.getPerso().getEtat());
			...
			}else if(monde.getPerso().getForm() == Form.LIGHTFORM && monde.getAnime().get(i) instanceof Monstre && persoRect.overlaps(monde.getAnime().get(i).getCadre())) {
