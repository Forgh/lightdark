...
//Enlever les null et mettre les SCL
	private void chargerCollision(){
		...
		for(int x = 0;x<=monde.getNiveau().getLargeur();x++){
			for(int y= 0;y<=monde.getNiveau().getHauteur();y++){
				...
				else if (this.perso.getForm() == Perso.Form.SHADOWFORM && monde.getNiveau().getCollisionWithLight(x, y) != null){ // (x>=dX) && (x<= fX) && (y>=dY) && (y<= fY)){
	...
	public void feuPresse(int x, int y, int w, int h) {
		 ...
		 */
		directionTir.x = v.x; // on evite de passer par zéro (bloquant)
		directionTir.y =  v.y;
